...
| **클릭스루** | 게임 플레이 방해 없는 클릭 통과 모드 |
| **테마 지원** | Midnight, Abyss, Deva, Asmodian, Flame, Glass 6가지 테마 |
| **전역 단축키** | 토글/새로고침/컴팩트/탭전환 전역 단축키 설정 |
| **자동 업데이트** | GitHub Releases 기반 자동 업데이트 확인 및 적용 |
| **전투 기록 업로드** | a2viewer.co.kr 사이트 연동 (동의 후 사용 가능) |

---

## 스크린샷

> 오버레이 UI는 WebView2(HTML/CSS/JS) 기반으로 렌더링됩니다.
...
# 2. Visual Studio 2022에서 솔루션 열기
start Aion2DPSViewer.sln

# 3. NuGet 패키지 복원
# Visual Studio 메뉴: 도구 → NuGet 패키지 관리자 → 솔루션용 패키지 복원
# 또는 패키지 관리자 콘솔에서:
...
| 패키지 | 버전 | 용도 |
|---|---|---|
| `Microsoft.Web.WebView2` | 1.0.2849+ | 오버레이 UI 렌더링 |
...
```
%APPDATA%\A2Viewer\
├── app_settings.json       # 사용자 설정 (단축키, 테마, 스케일 등)
...
- Npcap 드라이버가 필요하며, 관리자 권한으로 실행해야 합니다.
...
주요 변환 내용:
- `Microsoft.Web.WebView2` 유지 (net45 호환 버전)
